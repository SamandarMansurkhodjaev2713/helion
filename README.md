...
- **Хиро в трёх монтажах, один сценарий** — на десктопе прокрутка управляет
  `currentTime` видео с инерцией. На телефоне видео нет вовсе: скраб
  26-мегабайтного файла кладёт мобильный декодер, поэтому тот же дубль играется
  покадрово (72 кадра WebP, ~1.6 МБ, прогрессивная загрузка от каждого 8-го
  кадра к полной плотности). При `prefers-reduced-motion` — статичный кадр.
